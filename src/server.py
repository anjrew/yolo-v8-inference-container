...
def decode_image(image_data: bytes) -> cv2.imread:
    # Convert bytes to a numpy array
    nparr = np.frombuffer(image_data, "uint8")
    # Decode image
...
def server(host="0.0.0.0", port=5000):
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        with conn:
            ...
            image_size: int = struct.unpack(">L", conn.recv(4))[
                0
            ]  # Assuming the first 4 bytes indicate the image size
            raw_image_byte_data: bytes = conn.recv(image_size)

            image_data = decode_image(raw_image_byte_data)

            # Process the image to get bounding boxes and labels
            bounding_boxes = process_image(image_data)
